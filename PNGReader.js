...
PNGReader.prototype.decodeCgBI = function(chunk) {
  this.isCgBI = true;
};
...
PNGReader.prototype.decodePixels = function(callback) {
  ...
  inflate(data, this.isCgBI, function(err, data) {
...
PNGReader.prototype.interlaceNone = function(data) {
  ...
  png.pixels = this.isCgBI ? repairCgBIPixels(pixels) : pixels;
};

function repairCgBIPixels(pixels) {
  var bArr = new Uint8Array(pixels);
  var len = bArr.byteLength / 4;
  for (var i = 0; i < len; i++) {
    var buffer = [bArr[4 * i + 2], bArr[4 * i+ 1], bArr[4 * i], bArr[4 * i + 3]];
    bArr.set(buffer, 4 * i);
  }
  return bArr;
}
...
PNGReader.prototype.parse = function(options, callback) {
  ...
  try {
    ...
    while (this.i < this.bytes.length) {
      ...
      if (type != 'CgBI' && (type == 'IHDR' && options.data === false || type == 'IEND')) break;
    }
